...
const mg = new Mailgun({ apiKey: config.mailgun.apiKey, domain: config.mailgun.domain, url: config.api.mailgunUrl });
...
async function addMemberToList(member) {
  try {
    const newMember = { subscribed: true, address: member.email, name: member.name };
...
router.post('/api/sendEmailData', isAdmin, async (req, res) => {
  try {
    ...
    switch (recipientType) {
      case 'specific':
        emailPromises = specificEmails.map(email => sendEmailPromise(email, emailData));
        break;
      case 'subscribers':
      ...
      case 'members':
        ...
        emailPromises = members.map(member => sendEmailPromise(member.email, emailData));
        break;
    }
...
router.post('/api/subscribeToNewsletter', async (req, res) => {
  ...
  try {
    ...
    const alreadyExistsPattern = /Failed to add member: Address already exists '(.+)'/;
    if (alreadyExistsPattern.test(e.message)) {
      console.log(e.message); // Log the specific error message for debugging
      res.status(409).send(`The email address ${email} is already subscribed to the newsletter.`);
    } else {
